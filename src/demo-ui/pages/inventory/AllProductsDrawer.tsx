...
function ProductRow({ product }: { product: Product }) {
  ...
  const marginPct = Math.round(((product.price - product.costPrice) / product.price) * 100);

  return (
    <Stack direction="row" sx={{ alignItems: 'center', gap: 1.25, py: 1.25 }}>
      <Box
        sx={{
          width: 40,
          height: 40,
      ...
      <Box sx={{ flex: 1, minWidth: 0 }}>
        ...
        <Typography sx={{ fontSize: 11.5, color: 'text.secondary' }} noWrap>
          {product.category} · {product.brand} · {product.soldLast30} sold/30d
        </Typography>
      </Box>

      <Stack sx={{ alignItems: 'flex-end', flexShrink: 0, gap: 0.5, minWidth: 96 }}>
        <Stack direction="row" sx={{ alignItems: 'baseline', gap: 0.5 }}>
          <Typography sx={{ fontSize: 13.5, fontWeight: 800 }}>{formatINR(product.price)}</Typography>
          <Typography sx={{ fontSize: 10, color: 'text.disabled' }}>{marginPct}%</Typography>
        </Stack>
        <Chip
          size="small"
          label={`${product.stock} · ${stockStatusLabel[status]}`}
          sx={{
            height: 18,
            fontSize: 9.5,
            fontWeight: 800,
...
export function AllProductsDrawer({ open, onClose }: AllProductsDrawerProps) {
            ...
            return (
              <Chip
                ...
                sx={{
                  ...
                  color: active ? '#fff' : 'text.primary',
                  border: '1px solid rgba(11,15,26,0.06)',
                  '&:hover': { filter: active ? 'brightness(1.08)' : undefined, background: active ? undefined : 'rgba(11,15,26,0.08)' },
                }}
              />
            );
          })}
